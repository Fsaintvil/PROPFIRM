...
function Test-ProcessActive {
    ...
    if (-not (Test-ProcessAlive -Pid $Pid)) { return $false }
    ...
    for ($i=0; $i -lt $maxAttempts; $i++) {
        ...
        try {
            ...
            if ($content -and $content.pids -and $content.pids.Count -gt 0) {
                $alivePids = @()
                foreach ($p in $content.pids) { if (Test-ProcessAlive -Pid $p) { $alivePids += $p } }
...
function Remove-LockFile {
    if (Test-Path $lockFile) {
        try {
            ...
            if ($content.pids -contains $PID) { Remove-Item $lockFile -Force -ErrorAction SilentlyContinue }
        } catch { Remove-Item $lockFile -Force -ErrorAction SilentlyContinue }
    }
}
    ...
    while (-not (Test-Path $stopFlag)) {
        ...
        if ($nowUtc -ge $nextClose) {
            ...
            if (Test-Path $pidFile) {
                Get-Content $pidFile | ForEach-Object { if (Test-ProcessAlive -Pid $_) { Stop-Process -Id $_ -ErrorAction SilentlyContinue } }
            }
        ...
        if (Test-Path $pidFile) {
            Get-Content $pidFile | ForEach-Object {
                if (Test-ProcessActive -Pid $_ -LogFile $artifactsDir -TimeoutSec 180) { $running++ }
            }
        }
        ...
        if ($running -eq 0) {
            ...
            if (Test-Path $scriptPs) {
                try {
                    $md5 = (Get-FileHash $scriptPs -Algorithm MD5).Hash
                    Write-Output "💡 Script MD5=$md5"
