#!/usr/bin/env python3
"""Pass 3: ferme positions restantes ticket-par-ticket, SANS champ 'comment', cadence lente.
Usage: python tmp/close_remaining_positions_pass3.py [--pause SECONDS]
Écrit : artifacts/live_trading/close_all_positions_retry_pass3.json
Sécurité: vérifie la présence des variables d'environnement requises avant d'envoyer.
"""
import time
import json
import sys
...
OUTFILE = OUTDIR / 'close_all_positions_retry_pass3.json'
...
# enforce required environment opt-ins
REQUIRED_ENVS = ['ALLOW_MT5_SEND', 'AUTO_APPLY', 'AUTO_DEPLOY', 'AUTO_LEARN', 'AUTO_ADAPT', 'AUTO_ENRICH']
missing = []
for v in REQUIRED_ENVS:
    val = os.environ.get(v)
    report['env_check'][v] = val
    if val != '1':
        missing.append(v)

if missing:
    report['errors'].append('missing_required_env_flags:' + ','.join(missing))
...
# iterate positions and attempt close (no 'comment')
