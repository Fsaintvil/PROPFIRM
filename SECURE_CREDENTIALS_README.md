...
-------------------------
Les opérations sensibles (rotation de credentials, purge d'historique) doivent
être réalisées en conformité avec `OPERATIONAL_RULES.md`. Documentez toute action
dans un PR et référencez la conversation opérationnelle qui a autorisé la
modification. Les variables d'environnement requises pour les envois live sont
précisées dans ce fichier opérationnel.
