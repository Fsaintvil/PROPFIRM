...
for p in list(positions or []):
    try:
        ...
        order_type = mt5.ORDER_TYPE_SELL if type_pos == mt5.ORDER_TYPE_BUY else mt5.ORDER_TYPE_BUY

        tick = mt5.symbol_info_tick(symbol)
        if tick is None:
            report['results'].append({'ticket': ticket, 'symbol': symbol, 'note': 'no_tick'})
            continue
        ...
        try:
            ...
            report['results'].append({'ticket': ticket, 'symbol': symbol, 'note': 'order_check_exception', 'error': str(e)})
            continue
        ...
        rec = {'ticket': ticket, 'symbol': symbol, 'request': request, 'order_check': check_dict}

        comment = check_dict.get('comment') if isinstance(check_dict, dict) else str(check_dict)
        ...
        try:
            send = _mt5_send_safe(request) if _mt5_send_safe else send_order(request, enforce_cadence=True)
        ...
        report['results'].append({'ticket': getattr(p, 'ticket', None), 'error': str(exc)})
