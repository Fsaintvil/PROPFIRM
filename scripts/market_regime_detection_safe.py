...
# sklearn.metrics.silhouette_score not used in safe detector
...
class MarketRegimeDetectorSafe:
    ...
    def extract_regime_features(self, df):
        ...
        if price_col is None or not _price_plausible(df[price_col]):
            ...
            close_like = [
                c
                for c in df.columns
                if "close" in c.lower() and _price_plausible(df[c])
            ]
            if close_like:
                price_col = close_like[0]
            else:
        ...
        features["trend_short"] = (features["sma_5"] - features["sma_20"]) / features[
            "sma_20"
        ]
        features["trend_long"] = (features["sma_20"] - features["sma_50"]) / features[
            "sma_50"
        ]
    ...
    def fit_clustering_fallback(self, features):
        ...
        # numerically-stable softmax over negative distances
        neg = -distances
        neg = neg - np.min(neg, axis=1, keepdims=True)
        ex = np.exp(neg)
        regime_probs = ex / ex.sum(axis=1, keepdims=True)
    ...
    def detect_regimes(self, df):
        ...
        if features.shape[0] == 0:
            # no data -> return empty safe structures
            empty_probs = np.empty((0, self.n_regimes))
            return {
                "regimes": np.array([]),
        ...
        # ensure columns length matches n_regimes
        cols = [f"{n}_prob" for n in self.regime_names]
        self.regime_probabilities = pd.DataFrame(
            regime_probs, index=features.index, columns=cols
        )
        self.current_regime = int(remapped[-1]) if len(remapped) > 0 else None
        return {
            "regimes": remapped,
