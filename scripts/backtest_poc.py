...
def run_backtest(
    ...
    # apply entry costs on entries only
    prev_pos = np.concatenate([[0], pos[:-1]])
    entries = (pos == 1) & (prev_pos == 0)
    # subtract per-entry cost (transaction + slippage)
    ...
    # metrics
    total_return = float(np.prod(1 + strategy_ret) - 1)
    avg_ret = float(np.nanmean(strategy_ret))
