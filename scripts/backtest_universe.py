...
SYMBOL_MOMENTUM_PERIODS = {
    ...
    "AUDUSD": 24, "NZDUSD": 22, "XAUUSD": 30, "EURJPY": 20,
    "GBPJPY": 20, "USDJPY": 20, "BTCUSD": 20, "ETHUSD": 20,
    "JP225.cash": 20, "US500.cash": 20, "USOIL.cash": 20,
    # Crypto
    "SOLUSD": 20, "LNKUSD": 20, "BNBUSD": 20,
}
...
def batch_prod_signals(close, high, low, times, atr_arr, adx_arr, pos_di, neg_di, ema20, symbol):
    ...
    for i in range(MIN_BARS, n):
        ...
        if mom > 0:
            if pdi <= ndi * 0.8:
                dir_ok = False; di_sugg = "SELL"
        else:
            if ndi <= pdi * 0.8:
                dir_ok = False; di_sugg = "BUY"

        if mom > 0 and mom_abs >= tv:
            action = "BUY"; score = 0.50 + raw_score * 0.45
        elif mom < 0 and mom_abs >= tv:
            action = "SELL"; score = 0.50 + raw_score * 0.45
        ...
        signals[i] = {
            "action": action, "score": min(0.99, score),
            "atr": atr_v, "adx": adx_v,
            "regime": regime,
...
def batch_simple_signals(close, times, atr_arr, adx_arr):
    ...
    signals = np.full(n, None, dtype=object)
    for i in range(MIN_BARS, n):
        atr_v = atr_arr[i]; adx_v = adx_arr[i]
        if np.isnan(atr_v) or atr_v <= 0 or np.isnan(adx_v) or adx_v <= 0:
            continue
        if i < 21: continue
...
class SimTrade:
    __slots__ = ("symbol", "action", "entry", "sl", "tp", "atr_val", "regime",
                 "open_bar", "open_time", "direction", "closed", "result",
                 "profit_usd", "profit_usd_cost", "peak_price", "trailing_sl",
                 "partial_closed", "bars_held", "close_time", "close_price",
                 "lot", "_pip_size", "_pip_value", "_contract_size",
                 "_spread_pips", "commission_usd", "cost_pips")

    def __init__(self, symbol, action, entry, sl, tp, atr_val, regime,
                 bar_idx, bar_time, balance):
        self.symbol = symbol
        self.action = action
        self.entry = entry
        self.sl = sl
    ...
    def _calc_lot(self, entry, sl, balance):
        ...
        if dist > 0:
            ...
            if pips > 0:
                self.lot = risk / (pips * self._pip_value)
    ...
    def check_sl_tp(self, high, low, close, bar_idx, bar_time):
        ...
        if self.direction == 0:
            if low <= self.trailing_sl:
                self.close_price = self.trailing_sl; self.result = "SL"; hit = True
            elif high >= self.tp:
                self.close_price = self.tp; self.result = "TP"; hit = True
        else:
            if high >= self.trailing_sl:
                ...
                self.close_price = self.tp; self.result = "TP"; hit = True
        if hit:
            self.closed = True; self.close_time = bar_time
            self.bars_held = bar_idx - self.open_bar; self._calc_pnl()

    def _calc_pnl(self):
        usdpp = self.lot * self._pip_value
        if self.direction == 0:
    ...
    def to_dict(self):
        return dict(symbol=self.symbol, action=self.action, regime=self.regime,
                    entry=round(self.entry,5), sl=round(self.sl,5), tp=round(self.tp,5),
                    close_price=round(self.close_price,5), result=self.result,
                    ...
                    lot=round(self.lot,4), bars_held=self.bars_held,
...
def backtest_symbol(symbol, timeframe, df):
    ...
    for i in range(MIN_BARS, n):
        ...
        for lst in (open_s, open_p, open_pc):
            ...
            for t in lst:
                ...
                if not t.closed and i - t.open_bar > TIMEOUT_BARS.get(timeframe, 120):
                    t.closed = True; t.close_price = close[i]
                    t.close_time = times[i]; t.result = "TIMEOUT"
                    t.bars_held = i - t.open_bar; t._calc_pnl()
                if not t.closed:
                    still.append(t)
            lst[:] = still
        ...
        if ss is not None and bars_since >= 3:
            sd = ss["sl_atr"] * atr_v; td = ss["tp_atr"] * atr_v
            if ss["action"] == "BUY":
                sp = close[i] - sd; tp = close[i] + td
            else:
                sp = close[i] + sd; tp = close[i] - td
            if td/sd >= 2.0 if sd > 0 else False:
                if not any(t.action == ss["action"] for t in open_s):
                    t = SimTrade(symbol, ss["action"], close[i], sp, tp, atr_v,
                                 ss["regime"], i, times[i], INITIAL_BALANCE)
                    trades_simple.append(t); open_s.append(t); bars_since = 0

        # Prod
        sp_sig = sigs_prod[i]
        if sp_sig is not None and bars_since >= 3:
            sd = sp_sig["sl_atr"] * atr_v; td = sp_sig["tp_atr"] * atr_v
            if sp_sig["action"] == "BUY":
                sp = close[i] - sd; tp = close[i] + td
            else:
                sp = close[i] + sd; tp = close[i] - td
            if sd > 0 and td/sd >= 2.0 and not any(t.action == sp_sig["action"] for t in open_p):
                t = SimTrade(symbol, sp_sig["action"], close[i], sp, tp, atr_v,
                             sp_sig["regime"], i, times[i], INITIAL_BALANCE)
                trades_prod.append(t); open_p.append(t)
                tc = SimTrade(symbol, sp_sig["action"], close[i], sp, tp, atr_v,
                              sp_sig["regime"], i, times[i], INITIAL_BALANCE)
                trades_prod_cost.append(tc); open_pc.append(tc)
                bars_since = 0
...
def compute_metrics(closed, use_cost=False):
    ...
    # DD
    peak = INITIAL_BALANCE; dd_max = 0.0; bal = INITIAL_BALANCE
    ...
    return {"n": n, "wins": nw, "losses": n - nw,
            "win_rate": round(wr, 1), "total_pnl": round(tp, 2),
            "profit_factor": round(pf, 2), "max_drawdown_pct": round(dd_max, 1),
            "p_value": round(p, 4), "significant": p < 0.05 and wr > 50,
...
def avg_costs(closed):
    ...
    return {"avg_cost_pips": round(float(np.mean([t.cost_pips for t in closed])), 2),
...
def backtest_multi_tf(symbol):
    ...
    for tf in ("H1", "H4", "D1"):
        ...
        all_s.extend(s); all_p.extend(p); all_pc.extend(pc)
...
def main():
    ...
    parser.add_argument("--tf", choices=["H1", "H4", "D1", "ALL"], default="ALL",
                        help="Timeframe: H1, H4, D1, ou ALL (cumulé)")
    parser.add_argument("--min-trades", type=int, default=100,
                        help="Nombre minimum de trades pour qualifier un symbole")
    parser.add_argument("--top", type=int, default=15,
                        help="Afficher les N meilleurs symboles")
    args = parser.parse_args()

    data_dir = Path("data/historical")
    if not data_dir.exists():
        print("❌ data/historical/ introuvable"); sys.exit(1)
    ...
    for sym in all_symbols:
        ...
        results[sym] = {
            "simple": m_s, "prod": m_p, "prod_cost": m_pc,
            "costs": costs,
        ...
        print(f"  {emoji} {sym:12s}  Prod+Cost: {m_pc['n']:>5d} trades  "
    ...
    print(f"{'='*120}")
    print(f"  {'#':>2s} {'Symbole':12s} {'TF':>4s} {'Trades':>6s} {'WR':>5s}  {'PnL':>10s}  {'PF':>5s}  {'DD':>5s}  "
    ...
    for rank, (sym, r) in enumerate(ranked[:args.top], 1):
        ...
        print(f"  {rank:>2d} {sym:12s} {args.tf:>4s} {pc['n']:>6d} {pc['win_rate']:>4.1f}%{survive} "
    ...
    if survivors:
        for i, sym in enumerate(survivors, 1):
            ...
            print(f"  {i:>2d}. {sym:12s}  {pc['n']:>5d} trades  WR={pc['win_rate']:>5.1f}%  "
    ...
    report = {
        "metadata": {"timestamp": datetime.utcnow().isoformat(), "timeframe": args.tf,
                     "initial_balance": INITIAL_BALANCE, "risk_per_trade": RISK_PER_TRADE,
                     "slippage_pips": SLIPPAGE_PIPS, "commission_per_100k": COMMISSION_PER_100K,
                     "danger_hours": DANGER_HOURS, "min_trades": args.min_trades},
        "per_symbol": results,
        "ranking": [{"rank": i+1, "symbol": s, "pnl": results[s]["prod_cost"]["total_pnl"],
        ...
        "survivors": survivors,
        "totals": {"n": total_n, "wr": round(total_wr, 1), "pnl": round(total_pnl, 2),
                   "pf": round(total_pf, 2)},
    }

    def cj(o):
        if isinstance(o, dict): return {k: cj(v) for k, v in o.items()}
        if isinstance(o, list): return [cj(v) for v in o]
