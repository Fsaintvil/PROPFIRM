...
def main():
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Do not call MT5; create small sample CSV files for testing"
        ),
    )
    args = ap.parse_args()
    ...
    if args.dry_run:
        # create small sample CSVs with required columns for tests without MT5
        out_dir.mkdir(parents=True, exist_ok=True)
        import pandas as pd
        for sym in symbols:
            df = pd.DataFrame([
            ...
            out_csv = out_dir / f"{sym}_15m.csv"
            df.to_csv(out_csv, index=False)
            print(f"(dry-run) wrote {out_csv}")
        return
