...
Ce script recherche les appels `mt5.order_send(` dans les dossiers sources
...
on remplace chaque `mt5.order_send(request)` par
`_mt5_send_safe(request) if _mt5_send_safe else mt5.order_send(request)`.
