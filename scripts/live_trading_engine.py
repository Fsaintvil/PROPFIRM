...
try:
    from tools.broker_safety import clamp_sl_tp, validate_volume, normalize_price
            ...
            def _send_order():
                ...
                if result.retcode != mt5.TRADE_RETCODE_DONE:
                    raise MT5OperationError(
                        f"Order send failed, retcode={getattr(result, 'retcode', 'unknown')}"
                    )
                return result
                                    ...
                                    try:
                                        if BROKER_SAFETY_AVAILABLE:
                                            ...
                                            new_sl, new_tp, changed = clamp_sl_tp(
                                                current_price, stop_loss, take_profit, pdigits, min_stoplevel, point
                                            )
                                            if changed:
                                                self.logger.info(
                                        ...
                                        self.logger.debug("Broker preflight helpers failed, continuing without adjustments", exc_info=True)
