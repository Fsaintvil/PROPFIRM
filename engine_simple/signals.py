...
STRATS = MappingProxyType({
    ...
    "USDCHF": [{"tf": "M5", "period": 20, "thresh": 2, "sl": 1.5, "tp": 4.0}],
})

ATR_PERIODS = {"M5": 7, "M15": 10, "H1": 14, "H4": 20, "D1": 20}
...
class SignalGenerator:
    ...
    def analyze(self, symbol, overrides=None):
        ...
        h1_adx = self._calc_adx(hh, ll, cc)
        rsi_arr = rsi(cc)
        rsi_arr[-1] if len(rsi_arr) > 0 and not np.isnan(rsi_arr[-1]) else 50

        is_ranging = h1_adx < 20
        base_thresh = 2.5 if h1_adx >= 20 else 2.0
    ...
    def _compute_smc_signal(self, macro, structure, fvg_analysis, session_info, liq, multi_tf, symbol, rates_dict):
        ...
        session_w = session_info.get("session_weight", 0)
        factors["killzone"] = 0.5 if active_kz else 0
        factors["session_position"] = session_w
        ...
        if buy_score > sell_score and buy_score >= threshold:
            action = "BUY"
            entry_signal = True
            score = min(0.95, 0.40 + buy_score)
            confidence = min(0.90, 0.35 + buy_score * 0.8)
            ...
            action = "SELL"
            entry_signal = True
            score = min(0.95, 0.40 + sell_score)
            confidence = min(0.90, 0.35 + sell_score * 0.8)
        ...
        if overrides := getattr(self, '_overrides', None):
            pass
    ...
    def _eval_reversion(self, data, rsi_now, cfg):
        return None

    def _compute_indicators(self, c, h, lo, v, direction, atr_val):
        return {}

    def _check_rsi_divergence(self, price, rsi_arr):
        return 0

    def _aggregate_confidence(self, confidences, rates_dict):
        return {}
