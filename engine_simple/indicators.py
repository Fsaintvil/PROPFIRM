...
def chaikin_money_flow(
    close: np.ndarray, high: np.ndarray, low: np.ndarray, volume: np.ndarray, period: int = 20
) -> float:
...
def relative_volume(volume: np.ndarray, period: int = 50) -> float:
...
def obv_divergence(close: np.ndarray, volume: np.ndarray, period: int = 20) -> tuple[str, float]:
...
def stochastic_rsi(data: np.ndarray, period: int = 14, k: int = 3, d: int = 3) -> tuple[np.ndarray, np.ndarray]:
...
def ema_alignment(ema9: float, ema20: float, ema50: float, ema200: float, price: float) -> float:
...
def fibonacci_retracement(swing_high: float, swing_low: float, current_price: Optional[float] = None) -> dict[str, Any]:
...
def rsi_divergence(close: np.ndarray, rsi_values: np.ndarray, lookback: int = 20) -> dict[str, bool | float]:
...
def market_regime_features(
    high: np.ndarray, low: np.ndarray, close: np.ndarray, volume: np.ndarray, period: int = 50
) -> dict[str, float]:
    ...
    features["atr_ma"] = float(np.mean(atr_vals[-20:])) if len(atr_vals) >= 20 else 0

    obv_vals = obv(c, v)
    features["obv_slope"] = float(obv_vals[-1] - obv_vals[-20]) / max(abs(obv_vals[-20]), 1)

    rsi_vals = rsi(c)
    features["rsi"] = float(rsi_vals[-1]) if len(rsi_vals) > 0 and not np.isnan(rsi_vals[-1]) else 50
    features["rsi_slope"] = (
        float(rsi_vals[-1] - rsi_vals[-10]) if len(rsi_vals) >= 10 and not np.isnan(rsi_vals[-10]) else 0
    )

    macd_line, _, macd_hist = macd(c)
    features["macd_hist"] = float(macd_hist[-1]) if len(macd_hist) > 0 and not np.isnan(macd_hist[-1]) else 0
    macd_ok = len(macd_hist) >= 5 and not np.isnan(macd_hist[-5])
    features["macd_slope"] = float(macd_hist[-1] - macd_hist[-5]) if macd_ok else 0

    bb_upper, bb_mid, bb_lower = bollinger_bands(c)
    if len(bb_upper) > 0 and not np.isnan(bb_upper[-1]):
        bb_width = float(bb_upper[-1] - bb_lower[-1]) / max(bb_mid[-1], 0.0001)
        features["bb_width"] = bb_width
        features["bb_position"] = float(c[-1] - bb_lower[-1]) / max(bb_upper[-1] - bb_lower[-1], 0.0001)
    else:
        features["bb_width"] = 0
