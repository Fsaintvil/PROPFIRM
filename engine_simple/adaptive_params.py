...
class AdaptiveParameters:
    ...
    def __init__(
        self, symbol: str, lookback: int = 100, min_trades: int = 10, state_dir: str = "runtime"
    ):  # 🔧 16 Juil: 20→10 — activation plus rapide pour les nouveaux symboles
        self.symbol = symbol
    ...
    def _load_state(self):
        ...
        try:
            if self._state_file.exists():
                ...
                logger.debug(
                    f"  [ADAPTIVE] {self.symbol}: loaded {len(self._trades)} trades, WR={self._params.win_rate:.1%}"
                )
    ...
    def _recalculate(self):
        ...
        logger.debug(
            f"  [ADAPTIVE] {self.symbol}: WR={wr:.1%}, PF={pf:.2f}, "
