    ...
    def _load_recorded_positions(self):
        ...
        try:
            if RECORDED_POSITIONS_FILE.exists() and RECORDED_POSITIONS_FILE.stat().st_size > 10:
                ...
                logger.info(f"[TRACKER] Persist: {len(self._recorded_position_ids)} position_ids "
    ...
    def import_history(self):
        ...
        try:
            ...
            for d in deals:
                ...
                self.ftmo.record_trade_result(d.symbol, d.profit, historical=True)
                ...
                recorded += 1
            if recorded > 0:
                logger.info(f"[TRACKER] Import historique: {recorded} trades fermés importés "
                           f"(FTMO seulement, pas perf monitor)")
        ...
        for p in our:
            ...
            if p.ticket not in self._position_meta:
                ...
                meta = dict(
                    ...
                    sl=p.sl, tp=p.tp, lot=p.volume, regime=regime,
    ...
    def check_closed(self):
        ...
        for ticket in closed:
            ...
            if len(self._recorded_deals) >= self._max_recorded:
                self._recorded_deals = OrderedDict(
                    list(self._recorded_deals.items())[-self._trim_target:])
                ...
                try:
                    ...
                    if direct and len(direct) > 0:
                        for d in direct:
                            if d.profit != 0:
                                closing = d
                                logger.info(f"  [TRACKER] Found closing deal via direct lookup: "
                                           f"{closing.profit:.2f}")
                                break
                except Exception as e:
            ...
            if closing is None:
                logger.info(f"  [TRACKER] Ticket {ticket} ferme sans historique MT5 "
                           f"(gap de session — normal apres reconnexion)")
            ...
            self.ftmo.record_trade_result(closing.symbol, closing.profit, historical=is_historical)
