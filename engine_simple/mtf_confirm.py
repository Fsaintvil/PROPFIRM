...
class MultiTimeframeConfirmer:
    ...
    def confirm(self, df_signal: pd.DataFrame, df_higher: pd.DataFrame,
                action: str) -> tuple[bool, float]:
    ...
    def confirm_multi(self, df_signal: pd.DataFrame,
                      df_higher_list: list[pd.DataFrame],
                      action: str) -> tuple[bool, float, dict]:
...
def confirm(df_signal: pd.DataFrame, df_higher: pd.DataFrame,
            action: str) -> tuple[bool, float]:
    """Confirme un signal (fonction convenience)."""
    return _default_mtf.confirm(df_signal, df_higher, action)

def confirm_multi(df_signal: pd.DataFrame,
                  df_higher_list: list[pd.DataFrame],
                  action: str) -> tuple[bool, float, dict]:
