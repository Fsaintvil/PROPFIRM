...
# MT5Connector pour get_rates (pas une fonction standalone)
try:
...
def check_adx():
    ...
    for sym in symbols:
        try:
            rates = connector.get_rates(sym, "H1", 30)
...
def decision(force_check=False):
    ...
    if force_check or (now - snapshot_ts > ADX_SNAPSHOT_TTL):
        ...
        ratio = sum(1 for d in state.get("adx_snapshot", {}).values() if d.get("low")) / max(
            len(state.get("adx_snapshot", {})), 1
        )
