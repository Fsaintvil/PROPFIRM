...
try:
    ...
    detect_fvg = detect_liquidity_sweep = filter_active_fvgs = fvg_score = None
...
class AdaptiveEngine:
    def __init__(self, mt5, calibration_path=None):
        ...
        self.dl = DLEnsemble() if DLEnsemble is not None else None
        # ML Ensemble (RF/XGB/LGBM) désactivé — info-only à 45%, 581 MB RAM pour rien
        self.ml = None
        self.lgb = None  # LightGBM désactivé (poids=0)

        # Bypass MetaLearner when only 1 model is active (no ensemble possible)
        self._meta_active = self.dl is not None and self.dl.available
    ...
    def analyze(self, symbol, rates_dict, signal, trade_stats=None):
        ...
        active_fvgs = []  # default: no FVGs (used at line 694 unconditionally)
        if detect_fvg is not None:
        ...
        if detect_liquidity_sweep is not None and h4_rates is not None and len(h4_rates) >= 10:
        ...
        # Ne hardcoder que si le signal ne les a pas (fallback régime)
