...
def send_order(
    ...
    # Operational checks: ensure required env flags are set before any live send
    try:
        from src.utils.ops_checks import require_env_flags_or_raise

        require_env_flags_or_raise()
    except Exception as e:
        # Fail-fast: do not attempt live sends when operational flags are missing
        raise Mt5OrderError(f"Operational environment check failed: {e}")
    ...
    try:
        if s_info is not None:
            if "price" in request and request.get("price") is not None:
                request["price"] = _round_price_by_digits_or_point(
                    request.get("price"), s_info
                )
            if "sl" in request and request.get("sl") is not None:
                request["sl"] = _round_price_by_digits_or_point(
                    request.get("sl"), s_info
                )
            if "tp" in request and request.get("tp") is not None:
                request["tp"] = _round_price_by_digits_or_point(
                    request.get("tp"), s_info
                )
    except Exception:
    ...
    try:
        if s_info is not None:
            ...
            vol_step = getattr(s_info, "volume_step", None) or getattr(
                s_info, "trade_contract_size", None
            )
        else:
            min_vol = None
            vol_step = None
    ...
    try:
        ...
        for k in ("price", "sl", "tp", "volume"):
            if k in req_copy:
                try:
                    req_copy[k] = (
                        float(req_copy[k]) if req_copy[k] is not None else None
                    )
                except Exception:
                    pass
    ...
    # By default cadence enforcement can be enabled via env var
    # ENFORCE_ORDER_CADENCE=1 (useful in production). Tests remain unaffected.
    env_flag = os.getenv("ENFORCE_ORDER_CADENCE", "0")
    enforce_effective = bool(enforce_cadence) or env_flag == "1"
    if enforce_effective:
        try:
            # Use the central order_cadence util when available
            try:
                from src.utils import order_cadence as order_cadence

                sym = request.get("symbol")
                if sym and not order_cadence.can_send(sym, cooldown_s=930):
                    raise Mt5OrderError(f"Cadence blocked for symbol {sym}")
            except Mt5OrderError:
                raise
            except Exception:
                # If cadence util fails unexpectedly, proceed (fail-open)
                pass
        except Mt5OrderError:
            raise
        except Exception:
            # If cadence util fails, we log and continue (fail-open)
            logger.debug("order_cadence local check failed, proceeding", exc_info=True)
    ...
    if hasattr(mt5, "TRADE_RETCODE_DONE") and retcode != getattr(
        ...
        try:
            ...
            logger.warning(
                "MT5 order_send failed: retcode=%s, comment=%s", retcode, comment
            )
        raise Mt5OrderError(
            f"Order send failed, retcode={retcode}, comment={comment}, order={order_id}"
        )

    # Record successful send cadence (best-effort, write artifacts file)
    try:
        ...
        sym = symbol
        if sym:
            lf = Path("artifacts") / "live_trading" / "last_send_by_symbol.json"
            try:
                if lf.exists():
                    state = json.loads(lf.read_text(encoding="utf-8"))
                else:
                    state = {}
                state[sym] = float(time.time())
                lf.parent.mkdir(parents=True, exist_ok=True)
                lf.write_text(json.dumps(state, indent=2), encoding="utf-8")
            except Exception:
                pass
    except Exception:
        pass
