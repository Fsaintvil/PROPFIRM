...
def make_ftmo(mt5_mock):
    return FTMOProtector(mt5_mock, dict(
        ...
        COOLDOWN_MINUTES=cfg.COOLDOWN_MINUTES, MAX_DAILY_LOSS_PCT=cfg.MAX_DAILY_LOSS_PCT,
        INITIAL_BALANCE=200000, MAX_DD_PCT=cfg.MAX_DD_PCT,
        PROFIT_TARGET_PCT=cfg.PROFIT_TARGET_PCT, CONSISTENCY_MAX_PCT=cfg.CONSISTENCY_MAX_PCT,
        MIN_TRADING_DAYS=cfg.MIN_TRADING_DAYS, MAGIC=cfg.ROBOT_MAGIC,
        MAX_SPREAD_POINTS=cfg.MAX_SPREAD_POINTS, MAX_RISK_AMOUNT=cfg.MAX_RISK_AMOUNT,
...
class TestPositionTracker:
    ...
    def test_detect_closed_position(self):
        ...
        pos_cache.get.side_effect = [
            [MagicMock(ticket=1, magic=cfg.ROBOT_MAGIC, type=0, symbol="EURUSD",
                      price_open=1.10, sl=1.095, volume=0.05, comment="ADAPT_RAN", profit=0)],
    ...
    def test_track_non_robot_position_ignored(self):
        ...
        pos_cache = MagicMock()
        pos_cache.get.return_value = [
            MagicMock(ticket=2, magic=999999, type=0, symbol="EURUSD",
                     price_open=1.10, sl=1.095, volume=0.05, comment="", profit=0)
        ]
        tracker = PositionTracker(ftmo, journal, adaptive, pos_cache, mt5=mt5)
        tracker.init_tickets()
        tracker.track_new()
...
class TestATRTrailing:
    def _make_buy_pos(self, ticket=1, entry=1.1000, current=1.1050, sl=1.0980):
        pos = MagicMock(ticket=ticket, type=0, symbol="EURUSD",
                       price_open=entry, price_current=current, sl=sl,
                       volume=0.05, comment="ADAPT_RAN", magic=cfg.ROBOT_MAGIC)
        return pos

    def _make_sell_pos(self, ticket=1, entry=1.1000, current=1.0950, sl=1.1020):
        pos = MagicMock(ticket=ticket, type=1, symbol="EURUSD",
                       price_open=entry, price_current=current, sl=sl,
                       volume=0.05, comment="ADAPT_RAN", magic=cfg.ROBOT_MAGIC)
        return pos

    def test_trailing_no_update_below_threshold(self):
        mt5 = make_mock_mt5()
        ftmo = make_ftmo(mt5)
        ftmo._get_atr = MagicMock(return_value=0.005)
        pos = self._make_buy_pos(current=1.1010)  # profit = 1.0010-1.1000 = 0.001, 0.2 ATR
        ftmo._check_step_trailing(pos)
        assert mt5.update_sl.called is False

    def test_trailing_updates_at_first_level(self):
        mt5 = make_mock_mt5()
        ftmo = make_ftmo(mt5)
        ftmo._get_atr = MagicMock(return_value=0.005)
    ...
    def test_trailing_respects_regime_levels(self):
        ...
        result.retcode = 10009
        mt5.update_sl.return_value = result
        pos = self._make_buy_pos(current=1.1200)  # profit = 0.02, 4.0 ATR > 3.0
        ftmo._check_step_trailing(pos)
        assert mt5.update_sl.called
        new_sl = mt5.update_sl.call_args[0][1]
        expected_sl = round(1.1200 - 0.20 * 0.005, 5)  # level 3: 0.20 * ATR
        assert abs(new_sl - expected_sl) < 0.0001

    def test_trailing_sell_position(self):
        mt5 = make_mock_mt5()
        ftmo = make_ftmo(mt5)
        ftmo._get_atr = MagicMock(return_value=0.005)
        ftmo.position_regime = {"1": "RANGING"}
    ...
    def test_trailing_high_vol_wide_dist(self, mock_rand):
        ...
        assert mt5.update_sl.called
        new_sl = mt5.update_sl.call_args[0][1]
        expected_sl = round(1.1080 - 1.00 * 0.005, 5)  # HIGH_VOL level 1: 1.00
        assert abs(new_sl - expected_sl) < 0.0001
...
class TestPartialTP:
    def _make_buy_pos(self, ticket=1, entry=1.1000, current=1.1150, sl=1.0950, tp=1.1200):
        pos = MagicMock(ticket=ticket, type=0, symbol="EURUSD",
                       ...
                       volume=0.10, comment="ADAPT_RAN", magic=cfg.ROBOT_MAGIC)
        pos.profit = 25.0
        return pos
