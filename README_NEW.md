...
Référence centrale des règles: `OPERATIONAL_RULES.md` — consultez-le avant toute action.
Toujours se référer aux README du dépôt et à la conversation opérationnelle en cours avant d'agir.

Résumé rapide des obligations (voir `OPERATIONAL_RULES.md` pour détails):
- Ne pas créer de nouveaux scripts sans approbation explicite; modifier/étendre les scripts existants.
- Backtests 7 ans obligatoires (appliquer aux symbols, ex: BTCUSD, ETHUSD, XAUUSD, USDCAD, AUDNZD, EURJPY, GBPCHF, NZDJPY, EURUSD, EURAUD, US500.cash, JP225.cash et tous les autres si possible).
- Multi-timeframe standard: M15, M30, H1, H4, D1.
- Risk/Reward minimum cible: **1.5:3** (documenter l'application dans chaque stratégie).
- Toujours utiliser modèles IA et valider avec au moins un backtest 7 ans ET/OU 7 indicateurs techniques ET/OU 7 indicateurs institutionnels.
- Prioriser PowerShell pour commandes opérationnelles sous Windows; Python pour implémentation.

Ces règles réduisent le risque opérationnel et facilitent l'auditabilité des actions en production.