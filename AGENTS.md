...
> - **Fix 2 — XAUUSD threshold_ranging 2.0→2.5** (`strategy.py` L.103) : WR 21% sur 14 trades en 72h, trop de faux breakouts en ranging. Aligné sur trending pour filtrer les signaux faibles. **Actif immédiatement** (strategy.py hot-reloadé par signal).
> - **Fix 3 — LOT SAFETY clamp** (`ftmo_protector.py` L.1872) : le calcul risk_amount/risk_per_01 produisait des lots 28-30× le max pour NZDUSD/SOLUSD/BTCUSD. Clamp `lot = max_lot` ajouté AVANT le MAX_LOT_ABSURDITY_FACTOR. 21 occurrences/72h éliminées.
> - **Tests** : 1225 passed, 33 skipped (aucune régression). Robot PID 31564 vivant, 2 positions XAUUSD, DD 0%, Balance $200,844.
> - **Métriques GR** (140 trades depuis 13/08) : PnL +$1,046, WR 51%, Max DD 0.28%. **Edges** : BTCUSD +$774 (WR 68%, 34T), XAUUSD +$291 (WR 32%, 22T), GBPUSD +$29 (WR 75%, 8T). **Pertes** : AUDUSD −$38, USDCAD −$13, NZDUSD −$7.
> - **Pattern time_stops** : 8/10 derniers trades = time_stop → les trades n'atteignent pas le TP 7.5×ATR. Le fix 2h réduira les pertes mais le TP peut être trop éloigné pour certains symboles.
> - **Anomalies résiduelles** : GIL deadlock 2h32 (non corrigé, peut survenir), NZDUSD WR 14% post-GR (pertes faibles $0.20-$0.60), heartbeat stale 13 occurrences (non critique).

> **Mise à jour 21 Août 2026 (01:15)** : 🔬 **ANALYSE APPROFONDIE STRATÉGIE + 4 OPTIMISATIONS APPLIQUÉES (XAUUSD SL, BTCUSD lot, ADX per-symbol, doc min_score)** —
> - **Analyse complète** (strategy.py + signal_pipeline.py + signal_validator.py + regime.py croisés avec données live) : mécanique MOM20x3 saine (NaN guard, ATR-adaptatif, ADX slope, DI, pullback, HTF, volume). **Découverte clé : le min_score global 0.65 est un filtre INDIRECT** — un signal qui franchit juste le seuil technique (mom = seuil×ATR) a déjà score = 0.35 + 0.5×0.60 = 0.65 automatiquement → le plancher ne bloque QUE les signaux pénalisés par les ajustements pipeline (OBV ×0.70, phase1d ×0.75, SELL ×0.90, volume ×0.75). Le VRAI filtrage = ADX slope / DI / pullback / HTF / volume / régime strict.
> - **Fix 1 — XAUUSD SL 1.5→2.0×ATR** (strategy.py + YAML) : le SL 1.5 était **inadapté à la volatilité réelle** (ATR H4 live = $36.95, SL ≈ $55 vs mouvements or $50-100/jour) → **10 SL sur 13 trades GR (WR 15.4%, −418$)**. TP 6.0 conservé (RR 3.0). Cause racine n°1 des pertes GR identifiée et corrigée.
> - **Fix 2 — BTCUSD max_lot 0.03→0.06** (YAML) : BTCUSD = **SEUL edge prouvé statistiquement** (GR: 24 trades, WR 75%, PF 471, +263.50$, p=0.0085) → exposition doublée en collecte démo.
> - **Fix 3 — ADX per-symbol aligné** (strategy.py L.933) : hardcode `>= 22` → `adx_thresh` par symbole (crypto = 20, forex/indices = 22). Un BTCUSD à ADX=21 était traité en RANGING (seuil 2.0 au lieu de 2.5). Aligné sur 3 endroits : `is_trending` (L.933), DI Override (L.1094), `_adx_strong` structure (L.975). Cohérent avec regime.py (hystérésis adx_thresh/−4).
> - **Fix 4 — Documentation min_score** : commentaire explicatif dans signal_validator.py (filtre indirect, pas direct) pour éviter les faux fixes futurs.
> - **Tests** : +0 nouveau, 1 test adapté (BTCUSD max_lot 0.06) → **1225 passed, 33 skipped**. Robot VIVANT (PID 28224, 2 positions BTCUSD +37.38$, cycle 1409) — pas de redémarrage nécessaire (strategy.py relu par signal, YAML hot-reload ~15 min).

> **Mise à jour 21 Août 2026 (00:45)** : 📊 **DÉCISION UTILISATEUR « COLLECTE DÉMO PROLONGÉE » + 2 FIXES CONFIG SOLUSD (audit approfondi config/paramètres)** —
> - **Décision utilisateur** : après le verdict GR REJETÉ (105 trades, WR 50.5%, PF 0.85), l'utilisateur choisit **collecte démo prolongée** — portefeuille 13 symboles actuel conservé, objectif **200-300 trades propres** pour confirmer statistiquement les edges par symbole avant toute décision de scaling/re-tentative. Rien n'est risqué (compte démo).
> - **Fix 1 — SOLUSD min_score 0.65 INERTE corrigé (bug réel trouvé par audit config)** : le fix 19/08 (YAML `min_score` 0.60→0.65) ne s'appliquait JAMAIS car `signal_validator.py` (L.122-126) avait un **hardcode** `MIN_SCORE_EXCEPTIONS = {"SOLUSD": 0.60}` ET `strategy.py:SYMBOL_CONFIG.SOLUSD.min_score` restait à 0.60 → `cfg_score = max(0.60, plancher 0.60) = 0.60` toujours. **Fix** : `strategy.py` SOLUSD min_score 0.60→0.65 **et** l'exception validator → 0.65. Impact : SOLUSD (WR 46.2% GR, 26 trades) filtre désormais les signaux faibles 0.60-0.64. Test dédié : `test_solusd_exception_floor_065`.
> - **Fix 2 — SOLUSD SL YAML aligné sur stratégie réelle** : YAML `sl_atr_trending 2.0→2.5` / `sl_atr_ranging 1.5→2.0` (le SL réel utilisé venait déjà de strategy.py 2.5/2.0 — le YAML était trompeur, divergences inactives mais documentairement fausses).
> - **Audit complet config** : architecture de chargement confirmée = `default.yaml` + `production.yaml` mergés (le fichier production écrase : max_positions 30→8, per_trade 0.004→0.003, max_risk 800→600, min_score 0.6→0.65). Double source de vérité cartographiée : SL/TP/seuils/min_score = **strategy.py** (SymbolParamManager), spread/cooldown/max_lot/weekend = **YAML symbol_limits** (ftmo_protector). Divergences restantes documentées comme cosmétiques : `adx_slope_threshold` (lu que depuis strategy.py), `news_minutes_*` (filtre news global 15/10 min — per-symbol inerte dans les deux sens), `max_spread_points` YAML plus strict = effectif défensif.
> - **Performance GR par symbole (13 actifs)** : BTCUSD **seul edge prouvé** (24 trades, WR 75%, PF 471, +263.50$, p=0.0085) ; SOLUSD +42.49$ (PF 2.88) ; GBPUSD +29.33$ (WR 75%, 8 trades) ; US30 +4.79$ (3 trades). Perdants structurels GR : XAUUSD −418.11$ (WR 15.4%!), AUDUSD −38.34$, USDCAD −12.86$, USDJPY −2.18$, NZDUSD −4.95$. Raisons de sortie GR : SL 82 trades (−240$), time_stop 19 trades 0% WR (−133$), TP 8 trades (+213$).
> - **Tests** : +1 (SOLUSD floor 0.65) → **1225 passed, 33 skipped**. Robot inchangé (PID 29132, watchdog 17876) — pas de redémarrage nécessaire (hot-reload ~15 min propagera les valeurs).

> **Mise à jour 20 Août 2026 (20:45)** : 📉 **EURUSD max_lot 0.06→0.04 — dernier forex perdant structurel aligné (décision Robot Manager, pattern 18-19/08)** —
> - **Contexte** : EURUSD est le **dernier forex majeur à max_lot 0.06** alors que tous ses pairs perdants ont déjà été réduits (AUDUSD 0.13→0.05 le 18/08, USDCAD 0.06→0.04 + GBPUSD 0.17→0.06 + NZDUSD 0.11→0.05 le 19/08). Données GR nettoyée : **8 trades, WR 62.5% trompeur, PF 0.78, PnL −6.34$** (asymétrie R défavorable : winners +6.6/+9.0 vs losers −8.6/−11.5) ; toutes périodes : **86 trades, PnL −17.92$** ; backtest 19/08 : PF 0.74 (perdant structurel après coûts, comme tout le forex). Le WR élevé mais le PF < 1 = les winners sont petits (sorties au SL monté par BE progressif), les losers touchent le vrai SL.
> - **Fix** : `config/default.yaml:symbol_limits.EURUSD.max_lot 0.06→0.04` (seul changement — `max_lot` est lu dynamiquement via `_get_wr_based_max_lot` L.1733 depuis le dict `symbol_limits` hot-reloadé ~15 min, pas besoin de redémarrage). **Impact** : réduit l'hémorragie des −11.5$ à ~−7.7$ sans changer la collecte GR (le symbole continue de trader).
> - **Analyse complémentaire (surveillance 20:40)** : time_stops **100% perdants** (17/17 GR, −132.45$) = trades faibles jamais rentrés, **jamais un gagnant** → protection correcte, pas une fuite. Cooldown global 3 pertes = goulot principal de la collecte (263/348 rejets) mais protection volontaire conservée. Les 66 rejets spread BTCUSD = pics de volatilité (check points 150, correct). Indices US30/US100 bloqués par ratio ATR spread en marché étendu baissier (correct). **Pas de sur-ajustement horaire** (échantillons 2-15 trades/h trop petits — overfitting).

> **Mise à jour 20 Août 2026 (19:20)** : 🔧 **PURGE AUTOMATIQUE DES ADAPTED_PARAMS PRÉ-GR + FIX CURRENT_DD_PCT (fausses alertes DD) + VERDICT GR 104/100 — contexte de décision important** —
> - **Bug 1 — purge des params pré-GR INERTE (recommandation log-analyst 19/08 non effective)** : la purge manuelle des 5 adapted_params (GBPJPY/NZDUSD/US30.cash/USDJPY/USOIL.cash, 10-17 trades < 20) du 19/08 23:55 a été **annulée en ~19h** : le robot tournait avec les params en mémoire et les ré-écrivait au prochain save_state. Cause racine : `OnlineLearner._load_state` (L.237-242) et `AdaptiveEngine._load_calibration` (L.647-691) ne purgeaient QUE les symboles **sans** history, pas ceux avec history insuffisante (< min_trades = 20 valides) → les params pré-GR contournaient la garde `_update_params` L.418 à chaque redémarrage. **Impact live** : NZDUSD thresh=2.62, USDJPY 2.75, US30.cash 2.67 filtraient les signaux avec des seuils périmés (plus restrictifs, pas plus risqués — risk_mult=1.0). **Fix** : même garde que l'apprentissage (`n_valid < max(10, window//10)` → purge) appliquée aux **2 chemins** de restauration. Vérifié sur le vrai `ol_state.json` : les 5 params sont purgés au chargement, fichier ré-écrit propre (`adapted_params: []`).
> - **Bug 2 — current_dd_pct fallback 1.0 (=100% de DD!)** (`ftmo_protector.py` L.243-255) : quand `get_account_info()` échouait (blip MT5 transitoire, démarrage), le fallback retournait **1.0** → `risk_manager` comparait `1.0 >= max_dd_pct*0.8` → **13 fausses entrées dd_warning** dans reject_counter. **Fix** : mémorise `_last_dd_pct` (init 0.0) et le retourne en cas d'échec (fail-open doux — le check réel `_check_drawdown_limit` reste gardé par `if account:`). **Fix d'affichage** : `risk_manager.py:77` formatait un ratio en `%.1f%%` (0.05 → "0.1%") → `:.1%` correct.
> - **⚠️ VERDICT RÈGLE D'OR : 104/100 trades atteints mais WR 51.0% / PF 0.86 → REJETÉ (STOP définitif selon contrat 13/08)**. **Nuance critique — collecte polluée par des bugs désormais corrigés** : 24 trades à timestamp identique (doublons artefact) sur tout le log = **−554.84$** dont **−450.72$ XAUUSD du 20/08** (bug price-dedup inerte fixé à 09:00). **GR nettoyée des doublons : 97 trades, WR 52.6%, PF 1.78, +344.81$** — PF OK mais WR toujours < 60%. XAUUSD sans doublons = +32.61$ (WR 22% mais positive). La décision de scaling/re-tentative revient à l'utilisateur ; le robot tourne en collecte démo (rien n'est perdu, les données propres continuent de s'accumuler).
> - **Tests** : +3 (purge calibration ×1, purge ol_state ×1 avec BTCUSD ≥20 conservé, current_dd_pct last-known ×2) → **1224 passed, 33 skipped**.
> - **Robot redémarré** : PID **29132** (avant 21856), watchdog **17876**. SERVER_OFFSET **+3.00h** mesuré au démarrage (19:16:48). 2 positions BTCUSD reprises. Balance 199 696.3, DD 0.3%, WR 50%, consec 3. `[CAL] Restored adapted_params: 0 symbols (5 skipped)` = purge active en prod.
