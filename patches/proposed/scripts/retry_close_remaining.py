...
for entry in not_closed:
    ...
    if pos is None:
        ...
        results['attempts'].append({'ticket': ticket, 'symbol': symbol, 'note': note})
        continue
    ...
    if tick is None:
        results['attempts'].append({'ticket': ticket, 'symbol': symbol, 'note': 'no_tick'})
        continue
    ...
    try:
        ...
        results['attempts'].append({'ticket': ticket, 'symbol': symbol, 'note': 'order_check_exception', 'error': str(e)})
        continue
    ...
    attempt_record = {'ticket': ticket, 'symbol': symbol, 'request': request, 'order_check': check_dict}

    # if comment indicates invalid, skip
    comment = (check_dict.get('comment') if isinstance(check_dict, dict) else str(check_dict))
    retcode = check_dict.get('retcode') if isinstance(check_dict, dict) else None
