...
def propose_patch_for_file(path: Path) -> Tuple[int, str]:
    ...
    # For clarity, also replace common variant with named variable 'send' or 'result':
    # but the regex above handles general case where pattern includes paren
...
def main(argv: List[str] | None = None) -> int:
    ...
    ap.add_argument("--apply", action="store_true", help="Apply patches in-place (not recommended)")
    ...
    REPORT_OUT.write_text(json.dumps({"total_replacements": total, "files": report}, indent=2), encoding="utf-8")

    print(f"Proposals written under {PATCH_OUT}. Total replacements: {total}")
    if args.apply:
        print("--apply requested: applying patches in-place (overwrite originals). Be careful.")
        for rel in report.keys():
            src = ROOT / rel
            proposed = PATCH_OUT / rel
            if proposed.exists():
                src.write_text(proposed.read_text(encoding="utf-8"), encoding="utf-8")
